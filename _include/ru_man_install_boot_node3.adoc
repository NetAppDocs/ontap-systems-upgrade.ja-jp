...
WARNING: * HAペアでコントローラのアップグレードが完了したら、すぐにの設定を解除する必要があります `kmip.init.maxwait` 変数（Variable）：を参照してください link:ensure_controllers_set_up_correctly.html#unset_maxwait_manual["新しいコントローラが正しくセットアップされていることを確認します（手順11）"]。これを行わないと、停電が発生した場合にデータが失われる可能性があります。*
