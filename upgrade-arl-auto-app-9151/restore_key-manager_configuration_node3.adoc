...
summary: '必要に応じて、 `system controller replace`コマンドを使用して、ONTAP 9.15.1以降で導入されたコントローラへのアップグレード時にnode3のキーマネージャ構成を復元します。' 
---
...
.次の手順
node3 でキーマネージャの設定を復元した後、link:move_non-root_aggr_and_nas_data_lifs_node1_from_node2_to_node3.html["node1が所有する非ルートアグリゲートとNASデータLIFをnode2からnode3に移動する"]。
