...
summary: 'ONTAP 9.15.1 以降で導入されたコントローラーへのアップグレード時に、 `system controller replace`コマンドを使用して node2 の非ルートアグリゲートを node3 に再配置します。' 
---
...
.次の手順
非ルート集約とNASデータLIFをnode2からnode3に再配置した後、link:relocate-failed-or-vetoed-aggr-node3.html["障害が発生した、または拒否されたアグリゲートをnode3に再配置する"]。
