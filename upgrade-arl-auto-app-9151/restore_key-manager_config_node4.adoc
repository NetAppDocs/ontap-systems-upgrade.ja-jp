...
summary: '必要に応じて、 `system controller replace`コマンドを使用して、ONTAP 9.15.1以降で導入されたコントローラへのアップグレード時にnode4のキーマネージャ構成を復元します。' 
---
...
.次の手順
node4 でキーマネージャの設定を復元した後、link:move_non_root_aggr_and_nas_data_lifs_node2_from_node3_to_node4.html["node2が所有する非ルートアグリゲートとNASデータLIFをnode3からnode4に移動する"]。
