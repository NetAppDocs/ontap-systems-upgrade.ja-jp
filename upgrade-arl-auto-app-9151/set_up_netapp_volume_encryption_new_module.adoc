...
summary: '必要に応じて、ONTAP 9.15.1以降で導入されたコントローラにアップグレードする際に、 `system controller replace`コマンドを使用して、NVEまたはNAE用に新しいコントローラモジュールを設定してください。' 
---
...
.次の手順
新しいコントローラモジュールでNetApp Volume or Aggregate Encryptionを設定したら、link:decommission_old_system.html["旧システムを廃止する"]。
