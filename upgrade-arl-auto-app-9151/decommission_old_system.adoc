...
summary: 'NetAppに、 `system controller replace`コマンドを使用して、ONTAP 9.15.1以降で導入されたコントローラへのアップグレード後に旧システムが稼働していないことを通知します。' 
---
= 古いシステムの運用を停止
:hardbreaks:
:allow-uri-read: 
...
.手順
. link:other-references.html["参考資料"]を参照して_NetApp Support Site_にリンクしてログインしてください。
...
.次の手順
旧システムを廃止した後、link:resume_snapmirror_operations.html["SnapMirror処理を再開する"]。
