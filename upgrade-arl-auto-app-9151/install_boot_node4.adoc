...
summary: 'ONTAP 9.15.1以降で導入されたコントローラへのアップグレード時に、 `system controller replace`コマンドを使用してnode4をインストールして起動し、ONTAPソフトウェアをインストールします。' 
---
...
+
node4 プラットフォームの「インストールおよびセットアップ手順」、適切なディスク シェルフ ドキュメント、および「HA ペア管理」の手順に従って、次の接続をケーブル接続します。
...
. [[auto_check_4_step11]]ノード 4 がブート メニューで停止したら、ノード 4 で次のコマンドを実行して、ノード 2 のディスクをノード 4 に再割り当てします。
+
...
NOTE: 上記のコンソール出力例では、システムがAdvanced Disk Partitioning（ADP）ディスクを使用している場合、ONTAPからパートナーノード名の入力を求められます。

. [[auto_check_4_step27]]LOADERプロンプトで、ノードをブートします：
+
「 boot_ontap menu
...
.次の手順
node4をインストールして起動した後、link:verify_node4_installation.html["node4のインストールを確認する"]。
