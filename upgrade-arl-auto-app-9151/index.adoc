...
[role="lead"]
コントローラハードウェアをアップグレードするには、アグリゲートの再配置（ARL）方法がいくつかあります。このARL手順では、ONTAP 9.15.1以降で導入されたAFFおよびFASストレージシステムのコントローラHAペアを、既存のディスクとディスクに格納されているデータを維持しながら新しいコントローラにアップグレードする方法について説明します。


NOTE: この手順を使用して、MetroCluster FCまたはIP構成をアップグレードすることはできません。MetroCluster構成をアップグレードするには、link:other-references.html["参考資料"]を参照して、_MetroCluster アップグレードおよび拡張に関するドキュメント_にアクセスしてください。
...
.自動コントローラアップグレードプロセス
コントローラのアップグレード時には、既存のコントローラが、より新しい、あるいはより高性能なプラットフォームを実行する別のコントローラに交換されます。このコンテンツでは、自動ネットワークポート到達可能性チェックを利用してコントローラのアップグレードエクスペリエンスをさらに簡素化する、部分的に自動化された手順の手順について説明します。

.次の手順
このARLアップグレード手順について知った後、link:decide_to_use_the_aggregate_relocation_guide.html["この手順を使用するかどうかを決定する"]。
