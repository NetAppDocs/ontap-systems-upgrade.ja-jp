...
summary: 'ONTAP 9.15.1以降で導入されたコントローラへのアップグレード時に、 `system controller replace`コマンドを使用してnode3をインストールして起動し、ONTAPソフトウェアをインストールします。' 
---
...
. node3 がブート メニューで停止したら、node3 で次のコマンドを実行して、node1 のディスクを node3 に再割り当てします。
+
...
. 自動ブート中に `Ctrl-C`を入力して、ノードを `LOADER>`プロンプトで停止します。
...
.次の手順
node3をインストールして起動した後、link:verify_node3_installation.html["node3のインストールを確認する"]。
