...
summary: 'ONTAP 9.15.1以降で導入されたコントローラーへのアップグレード時に、 `system controller replace`コマンドを使用して、node2でHAペアを無効にし、node1をシャットダウンします。' 
---
= ノード 1 を撤去
:hardbreaks:
:allow-uri-read: 
...
.次の手順
node1を廃止した後、link:prepare_for_netboot.html["ネットブートの準備"]。
