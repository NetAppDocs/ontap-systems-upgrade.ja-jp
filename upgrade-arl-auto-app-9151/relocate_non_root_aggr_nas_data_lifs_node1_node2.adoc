...
summary: 'コマンドを使用してONTAP 9.15.1以降を実行しているコントローラをアップグレードする場合は、ルート以外のアグリゲートとNASデータLIFをノード1からノード2に移動し、ノード1のリソースをノード3に移動し `system controller replace` ます。' 
---
...
.次の手順
node1が所有する非ルート集約とNASデータLIFをnode2に移動した後、link:relocate-failed-or-vetoed-aggr-node2.html["障害が発生した、または拒否されたアグリゲートをnode2に再配置する"]。
