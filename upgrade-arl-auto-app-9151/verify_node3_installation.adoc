...
summary: 'ONTAP 9.15.1以降で導入されたコントローラにアップグレードする際に、 `system controller replace`コマンドを使用して、node1の物理ポートがnode3に正しくマッピングされていることを確認し、node3のネットワーク構成を復元します。' 
---
= ノード 3 のインストールを確認します
:hardbreaks:
:allow-uri-read: 
...
.このタスクについて
link:other-references.html["参考資料"]を参照して、_Hardware Universe_ にリンクし、新しいノードのポートに関する情報を取得してください。この情報は、このセクションの後半で使用します。

ノードのモデルによっては、物理ポートの配置が異なる場合があります。新しいノードが起動すると、ONTAPは自動的にクォーラムに参加するために、クラスタLIFをホストするポートを特定しようとします。

node1 の物理ポートが node3 の物理ポートに直接マッピングされない場合は、次のセクション<<restore_network_configuration_on_node3_9151,ノード 3 でネットワーク設定をリストア>>を使用してネットワーク接続を修復する必要があります。

node3 のインストールとブートが完了したら、正しくインストールされていることを確認する必要があります。node3 がクォーラムに参加するのを待ってから、再配置処理を再開する必要があります。

この手順のこの時点で、node3がクォーラムに参加するため、処理は一時停止します。



[[step-1-verify-that-node3-is-in-quorum]]
== 手順1：node3がクォーラムに参加していることを確認する

node3がクォーラムに含まれていることを確認してください。次に、コントローラ交換処理を再開します
...
. コントローラの交換処理は、介入のため一時停止し、次のメッセージが表示されます：
+
....
...
VLAN、インターフェイスグループ、ブロードキャストドメインの作成と再作成の詳細については、link:other-references.html["参考資料"] を参照して「ネットワーク管理」にリンクしてください。

.手順
. アップグレードされた node1 （ node3 ）にある物理ポートをすべて一覧表示します。
+
...
+
到達可能性修復が実行されると、ONTAPはポートを正しいブロードキャストドメインに配置しようと試みます。ただし、ONTAPがポートの到達可能性を判断できず、そのポートが既存のブロードキャストドメインのいずれにも属していない場合、ONTAPはこれらのポート用に新しいブロードキャストドメインを作成します。
...
. 次の手順に従って、ブロードキャストドメインに適切な物理ポートを割り当て `Cluster` ます。
+
...
----
+
次に、ポート「e9a」で削除されたVLANを「e9d」にリストアする例を示します。
...
.次の手順
node3のインストールを確認した後、link:restore_key-manager_configuration_node3.html["node3 上でキーマネージャの設定を復元する"]。
