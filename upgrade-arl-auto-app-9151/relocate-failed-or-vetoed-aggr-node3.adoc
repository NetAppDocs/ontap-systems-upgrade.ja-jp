...
summary: 'ONTAP 9.15.1以降を実行しているコントローラをアップグレードするときに、障害が発生したアグリゲートまたは拒否されたアグリゲートを手動でノード3に再配置します。 `system controller replace`コマンド。' 
---
...
.次の手順
失敗した、または拒否されたアグリゲートをノード3に移動した後、link:retire_node2.html["node2 を廃止する"]。
