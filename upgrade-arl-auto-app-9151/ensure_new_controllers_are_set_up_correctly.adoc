...
summary: 'HAペアを有効にし、node3とnode4のストレージ アクセスを確認し、ONTAP 9.15.1以降で導入されたコントローラへのアップグレード後に `system controller replace`コマンドを使用してアグリゲートの所有権を確認します。' 
---
...
. [[unset_maxwait_system_commands]]構成で自己暗号化ドライブを使用している場合は、変数をに設定して `kmip.init.maxwait` `off` （のように） link:install_boot_node4.html#auto_install4_step24["Node4 をインストールしてブートします（手順 24 ）"]、変数の設定を解除する必要があります。
+
...
.次の手順
新しいコントローラーが正しく設定されていることを確認した後、link:set_up_storage_encryption_new_module.html["新しいコントローラモジュールで Storage Encryption を設定する"]。
