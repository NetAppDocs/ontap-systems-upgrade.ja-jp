...
summary: アグリゲートの再配置を使用して AFF、ASA、または FAS コントローラハードウェアをアップグレードする方法について説明します。 
---
= ここから始めましょう：AFF、ASA、またはFASコントローラハードウェアのARLアップグレード手順を選択してください
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
アグリゲートの再配置（ARL）を使用することで、AFF、ASA、またはFASコントローラのハードウェアを無停止でアップグレードできます。コントローラハードウェアをアップグレードするその他の方法については、link:../upgrade/upgrade-decide-to-use-this-guide.html["ボリュームまたはストレージを移動してアップグレードします"]を参照してください。
...
== ONTAP 9.15.1以降で導入されたシステムへのアップグレード

アップグレードシナリオが以下のサポート対象システムマトリックスに記載されている場合は、link:../upgrade-arl-auto-app-9151/index.html["「system controller replace」コマンドを使用して、ONTAP 9.15.1以降で導入されたコントローラハードウェアにアップグレードします"]に移動してアップグレード手順を開始してください。
...
アップグレードシナリオが以下のサポート対象システムマトリックスに記載されている場合は、link:../upgrade-arl-auto-in-chassis/index.html["「system controller replace」コマンドを使用して、同じシャーシ内のコントローラモデルをアップグレードします"]アップグレード手順を開始します。
...
| AFF A70 | AFF A90用 | 9.18.1以降 


| AFF A30用 | AFF A50用 | 9.18.1以降 


| AFF A20用 | AFF A30、AFF A50 | 9.18.1以降 


| AFF C30 | AFF C60 | 9.18.1以降 


| FAS70を使用したチャンク アップロード署名要求がサポートされるようになりました。 | FAS90 | 9.18.1以降 
