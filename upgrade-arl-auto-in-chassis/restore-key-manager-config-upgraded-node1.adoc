...
.次の手順
アップグレードされたノード1でキーマネージャの設定を復元した後、link:move-non-root-aggr-nas-lifs-node1-from-node2-to-upgraded-node1.html["node1の非ルート・アグリゲートとNASデータLIFをnode2からアップグレード後のnode1に移動します"]。
