...
summary: ARLを使用して同じシャーシ内のコントローラモデルをアップグレードする場合は、必要に応じてストレージ暗号化を設定し、SSL証明書をインストールし、キー管理サーバをセットアップします。 
---
...
.次の手順
新しいコントローラーモジュールでストレージ暗号化を設定した後、link:set-up-netapp-volum-encryption-new-module.html["新しいコントローラモジュールでNetApp Volume EncryptionまたはAggregate Encryptionを設定"]。
