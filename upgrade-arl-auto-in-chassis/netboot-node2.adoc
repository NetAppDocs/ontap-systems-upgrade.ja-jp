...
summary: 同一シャーシ内でARLを使用してコントローラモデルをアップグレードする場合、対応する交換用システムモジュールを交換した後、node2をネットブートする必要があります。 
---
= ネットブート node2
:hardbreaks:
:allow-uri-read: 
...
.次の手順
node2をネットブートした後、link:cable-node2-for-shared-cluster-HA-storage.html["共有クラスタHAおよびストレージ用のケーブルnode2"]。
