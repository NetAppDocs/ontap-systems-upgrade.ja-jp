...
.このタスクについて
交換用システムモジュールを搭載したnode2を起動した後、正しくインストールされていることを確認します。node2がクォーラムに参加するまで待ってから、コントローラの交換操作を再開してください。
...
== 手順1：node2がクォーラムに参加していることを確認する

node2がクォーラムに参加していることを確認してください。次に、コントローラー交換作業を再開してください。
...
. コントローラーの交換が一時停止状態になったら、<<restore_network_configuration_on_node2,node2 でネットワーク設定をリストアします>>に進みます。
...
.このタスクについて
VLAN、インターフェースグループ、ブロードキャストドメインの作成と再作成の詳細については、link:other-references.html["参考資料"]を参照して_Network Management_コンテンツにリンクしてください。
...
.次の手順
node2のインストールを確認した後、link:restore-key-manager-config-node2.html["node2 上でキーマネージャの設定を復元する"]。
