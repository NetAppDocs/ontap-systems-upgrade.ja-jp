...
[role="lead"]
交換用モジュールを搭載した Node2 の起動準備が完了しました。サポートされている交換モジュールは、link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["サポートされるシステムマトリックス"]に記載されています。
...
. ノードがブート メニューで停止した場合は、node2 で次のコマンドを実行して、古い node2 ディスクを交換用の node2 に再割り当てします。
+
...
.次の手順
交換用システムモジュールを使用してnode2を起動した後、link:verify-node2-installation.html["node2のインストールを確認する"]。
