...
[role="lead"]
交換用モジュールを搭載した Node1 の起動準備が完了しました。サポートされている交換モジュールは、link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["サポートされるシステムマトリックス"]に記載されています。
...
+
手順の前半で記録したパスフレーズとバックアップ情報を入力します。link:complete-preparation-for-upgrade.html#manage-storage-encryption-using-okm["オンボードキーマネージャを使用してストレージ暗号化を管理します"]を参照してください。
...
.次の手順
交換用システムモジュールを使用してノード1を起動した後、link:verify-node1-installation.html["node1のインストールを確認する"]。
