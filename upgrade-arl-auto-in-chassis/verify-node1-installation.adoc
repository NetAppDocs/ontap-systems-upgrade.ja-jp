...
== 手順1：node1がクォーラムに参加していることを確認する

node1がクォーラムに含まれていることを確認してください。次に、コントローラー交換作業を再開してください。
...
. コントローラーの交換が一時停止状態になったら、<<restore_network_configuration_on_node1,ノード 1 でネットワーク設定をリストアします>>に進みます。




== ステップ2：node1でネットワーク構成を復元する
...
.次の手順
node1のインストールを確認した後、link:restore-key-manager-config-upgraded-node1.html["アップグレードされたnode1でkey-managerの設定を復元する"]。
