...
+
 `storage aggregate relocation start`、 `override-vetoes`、 `override-destination-checks`オプションの詳細については、link:other-references.html["参考資料"]を参照して_ONTAP 9 Command reference_にリンクしてください。
...
+
node2が交換されると、aggr_node_1はnode2ではなくnode1をホームノードとしてオンラインになります。
...
+
正しく再配置されなかったアグリゲートを識別するには、セクションlink:prepare-nodes-for-upgrade.html["ノードをアップグレードする準備をします"]で取得したnode1のホーム所有者を持つアグリゲートのリストを参照し、上記のコマンドの出力と比較します。

. ステップ1の出力を、セクションlink:prepare-nodes-for-upgrade.html["ノードをアップグレードする準備をします"]のノード1でキャプチャした出力と比較し、正しく再配置されなかったアグリゲートをメモします。
