...
summary: 対応する AFF A900 ノード 2 コントローラモジュールと NVS をスワップしたら、ネットブートが必要になることがあります。 
---
= ネットブート node2
:hardbreaks:
:nofooter: 
...
[role="lead"]
対応する AFF A900 ノード 2 コントローラモジュールと NVS をスワップしたら、ネットブートが必要になることがあります。ネットブートという用語は、リモート・サーバに保存された ONTAP イメージからブートすることを意味します。ネットブートの準備を行うときは、システムがアクセスできる Web サーバに、 ONTAP 9 ブート・イメージのコピーを配置する必要があります。
...
. AFF A900 コントローラモジュールで node2 が実行されてブートするのを待ち、次のようにブートメニューオプションを表示します。
+
[listing]
----
...
. node2 の LOADER プロンプトで、「 partner-sysid 」を確認する必要があります。node2 の場合 'partner-sysid' は node1 のものである必要がありますnode1 の「 partner-sysid 」を確認します。
+
