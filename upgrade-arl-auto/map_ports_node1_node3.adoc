...
ポート設定は、ノードのモデルによって異なる場合があります。元のノード上のポートとLIFの設定を、新しいノードの計画された使用方法と設定に適合させる必要があります。これは、新しいノードが起動時に同じ構成を再生するためです。つまり、node3 を起動すると、ONTAP は node1 で使用されていたのと同じポートでLIFをホストしようとします。

したがって、node1の物理ポートがnode3の物理ポートに直接マッピングされていない場合、ブート後にクラスタ、管理、およびネットワーク接続をリストアするには、ソフトウェア構成の変更が必要になります。さらに、node1のクラスタポートがnode3のクラスタポートに直接マッピングされていない場合、クラスタLIFを正しい物理ポートでホストするようにソフトウェア構成を変更するまで、node3はリブート時に自動的にクォーラムに再参加しない可能性があります。
...
+
link:other-references.html["参考資料"]を参照して、詳細については『ネットワーク管理』または『ONTAP 9 コマンド リファレンス』にリンクしてください。
