...
** network interface show -curr-node nodename __ role cluster、intercluster、node-mgmt、clustermgmt、data
** 'network port show -nodename_type physical`
...
** system node show -instance -node _nodename _`
** 'run-node_node_name sysconfig '
** storage aggregate show -node _nodename _`
