...
ただし、VシリーズシステムまたはFlexArray仮想化ソフトウェアを使用していて、ストレージアレイに接続されていて、ノード4にオンボードのFCポート、オンボードUTA / UTA2ポート、またはUTA / UTA2カードがない場合は、_Installに戻ってブートし、node4_に戻ってから再開する必要がありますlink:install_boot_node4.html#step22["手順 22"]。
...
** スキップ_ノード4のUTA / UTA2カードまたはUTA / UTA2オンボードポートを確認して設定するノード4にUTA / UTA2オンボードポートがない場合は、_に戻ってnode4_をインストールしてブートし、で再開します。link:install_boot_node4.html#auto_install4_step23["手順 23"]
...
+
A800にアップグレードする場合は、に進みますlink:reassign-node2-disks-to-node4.html#auto_check_4_step9["ノード2のディスクをノード4に再割り当て（手順9）"]。
