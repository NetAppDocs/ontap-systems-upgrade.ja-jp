...
したがって、node2の物理ポートがnode4の物理ポートに直接マッピングされていない場合、ブート後にクラスタ、管理、およびネットワーク接続をリストアするには、ソフトウェア設定の変更が必要です。また、node2のクラスタポートがnode4のクラスタポートに直接マッピングされていない場合、正しい物理ポートでクラスタLIFをホストするようにソフトウェア設定を変更するまで、node4はリブート時に自動的にクォーラムに再参加しない可能性があります。
...
+
link:other-references.html["参考資料"]を参照して、_Network Management_または_ONTAP 9 Command reference_にリンクし、_Configuring failover settings on a LIF_の詳細を確認してください。
