...
NetApp Aggregate Encryption（NAE）またはNetApp Volume Encryption（NVE）を使用してアップグレードするシステムのボリュームを暗号化している場合は、暗号化構成を新しいノードと同期する必要があります。キーマネージャを復元しない場合、ARLを使用してnode1のアグリゲートをnode2からnode3に再配置すると、暗号化されたボリュームがオフラインになります。
