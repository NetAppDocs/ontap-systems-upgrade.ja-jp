...
CAUTION: この段階では、MetroCluster設定が正常な状態ではなく、解決すべきエラーが発生している可能性があります。link:verify_health_of_metrocluster_config.html["MetroCluster 構成の健全性を確認"]を参照してください。
...
.このタスクについて
VLAN、インターフェースグループ、ブロードキャストドメインの作成と再作成の詳細については、link:other-references.html["参考資料"]にアクセスし、_Network Management_へのリンクを参照してください。
...
NetApp Aggregate Encryption（NAE）またはNetApp Volume Encryption（NVE）を使用してアップグレードするシステムのボリュームを暗号化している場合は、暗号化構成を新しいノードと同期する必要があります。キーマネージャを復元しない場合、ARLを使用してnode2のアグリゲートをnode3からnode4に再配置すると、暗号化されたボリュームがオフラインになります。
