...
* ただし、Vシリーズシステムまたはストレージアレイを搭載したFlexArray仮想化ソフトウェアを搭載したシステムで、node3にオンボードのFCポート、オンボードUTA / UTAポート、またはUTA / UTA2カードがない場合は、_Installに戻ってnode3_をブートし、で再開しますlink:install_boot_node3.html#auto_install3_step23["手順 23"]。
...
| は、 V シリーズシステムまたは FlexArray 仮想化ソフトウェアを搭載し、ストレージアレイに接続されています  a| 
** ノード 3 に UTA / UTA2 カードまたは UTA / UTA2 オンボードポートがある場合は、に進みます <<ノード 3 の UTA / UTA2 ポートを確認して設定してください>>。
** ノード3にUTA / UTA2カードまたはUTA / UTA2オンボードポートがない場合は、この手順をスキップし<<ノード 3 の UTA / UTA2 ポートを確認して設定してください>>て_Installに戻り、node3_ at resumeをブートしますlink:install_boot_node3.html#auto_install3_step23["手順 23"]。
...
. [[step14]]を実行してノードをブートメニューにブートし `boot_ontap menu`ます。AFF A800システムにアップグレードする場合は、に進みますlink:reassign-node1-disks-to-node3.html#auto_check3_step9["ノード1のディスクをノード3に再割り当て（手順9）"]。
