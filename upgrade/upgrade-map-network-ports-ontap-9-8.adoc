...
.. 到達可能性修復が実行されると、ONTAPはポートを正しいブロードキャストドメインに配置しようと試みます。ただし、ONTAPがポートの到達可能性を判断できず、そのポートが既存のブロードキャストドメインのいずれにも対応していない場合、ONTAPはこれらのポート用に新しいブロードキャストドメインを作成します。必要に応じて、新しく作成されたブロードキャストドメインのすべてのメンバーポートがインターフェースグループのメンバーポートになる場合は、それらのドメインを削除できます。ブロードキャストドメインを削除します（：）
+
...
. [[map_98_7]]ONTAP 9.8以降、ネットワークポート到達可能性修復手順中にポートがブロードキャストドメイン間で移動された場合、ONTAPはLIFのホームポートを自動的に変更します。LIFのホームポートが別のノードに移動された場合、または割り当てられていない場合、そのLIFは移動されたLIFとして表示されます。ホームポートが存在しなくなった、または別のノードに再配置された、移動されたLIFのホームポートを復元します。
+
