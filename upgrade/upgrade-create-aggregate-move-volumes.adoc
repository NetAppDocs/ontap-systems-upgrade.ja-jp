...
+
新しいアグリゲートと、ボリュームの移動元である古いアグリゲートは、どちらも同じSVMに属している必要があります。
...
.次の手順
アグリゲートを作成し、ボリュームを新しいノードに移動した後、link:upgrade-move-lifs-to-new-nodes.html["非SANデータLIFとクラスタ管理LIFを新しいノードに移動する"]。
