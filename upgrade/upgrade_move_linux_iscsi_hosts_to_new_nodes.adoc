...
LUNとボリュームは、依然として古いノード上に物理的にホストされています。LUNは旧ノードのインターフェース上でのみ報告されるため、ホストには旧ノードを経由するパスのみが表示されます。これを確認するには、ホスト上で `sanlun lun show -p`および `multipath -ll -d`コマンドを実行し、コマンドの出力結果を調べます。
...
.次の手順
Linux iSCSI ホストを新しいノードに移動した後、link:upgrade-create-aggregate-move-volumes.html["アグリゲートを作成し、ボリュームを新しいノードに移動します"]。
