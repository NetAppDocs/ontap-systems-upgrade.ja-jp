...
. ブートメニューのプロンプトで、 `wipeconfig`と入力してEnterキーを押します。
+
次の出力が表示されます：
...
----
+
システムは `wipeconfig`手順を開始し、再起動します。手順が完了すると、システムはブートメニューに戻ります。

. ブートメニューから「8」を入力してノードを再起動し、AUTOBOOT中にCtrl-Cを入力してLOADERプロンプトでノードを停止します。


.次の手順
新しいノードのデフォルト設定をリセットした後、link:upgrade-install-new-nodes.html["新しいノードをインストールします"]。
