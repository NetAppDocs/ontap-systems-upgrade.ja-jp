...
. [[map_98_5] データ LIF をホストする物理ポートのブロードキャストドメインメンバーシップを変更します。これは、 ONTAP 9.7 以前を使用してネットワークポートを _Map に示すように、手動で実行できます。 link:upgrade-map-network-ports-ontap-9-7-or-earlier.html#map_97_7["手順 7"]。推奨される方法は、 ONTAP 9.8 で導入された拡張ネットワーク到達可能性スキャンおよび修復手順を使用することです。次の手順 5 の手順（ a ）から（ g ）までを参照してください。
+
