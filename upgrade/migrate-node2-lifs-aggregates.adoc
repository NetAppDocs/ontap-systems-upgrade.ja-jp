...
summary: node2 をドライブシェルフに変換する前に、node2 上の論理インターフェイス（LIF）とデータアグリゲートを node1 に移行します。 
---
...
[role="lead"]
AFF A150、AFF A220、またはFAS2820のnode2をドライブシェルフに変換する前に、node2上の論理インターフェース（LIF）とデータアグリゲートをnode1に移行します。

.作業を開始する前に
次の要件を満たしていることを確認します。

* 元のコントローラと交換用コントローラのONTAPリリースバージョンとパッチバージョンが同じであること（可能な場合）。サポートされているONTAPリリースについては、 https://hwu.netapp.com["NetApp Hardware Universe の略"^]を参照してください。
...
.次の手順
link:convert-node2-to-drive-shelf.html["node2をドライブシェルフに変換し、node4に接続します"]
