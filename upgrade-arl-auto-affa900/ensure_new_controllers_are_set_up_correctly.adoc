...
. 構成にNetApp Storage Encryption（NSE）を使用していた場合、「setenv bootarg.storageencryption.support」コマンドを「kmip.init.maxwait`」変数を「true」に設定します（AFF A900またはFAS9500コントローラとNVRAMモジュールを搭載したin_bootnode2）。 link:boot_node2_with_a900_controller_and_nvs.html#boot_node2_step1["手順 1."]）を使用している場合は、変数をリセットする必要があります
+
