...
[role="lead"]
node2の物理ポートは、node4の物理ポートに正しくマッピングされなければならない。これにより、アップグレード後もnode4はクラスタ内の他のノードやネットワークと通信できるようになる。
...
.このタスクについて
ポート設定は、ノードのモデルによって異なる場合があります。元のノードのポートとLIF構成を、新しいノードの構成案と互換性のあるものにする必要があります。これは、新しいノードが起動時に同じ構成を再生するためです。つまり、node4を起動すると、Data ONTAPはnode2で使用されていたのと同じポートでLIFをホストしようとします。

したがって、node2の物理ポートがnode4の物理ポートに直接マッピングされていない場合、ブート後にクラスタ、管理、およびネットワーク接続をリストアするには、ソフトウェア構成の変更が必要です。また、node2のクラスタポートがnode4のクラスタポートに直接マッピングされていない場合、正しい物理ポートでクラスタLIFをホストするようにソフトウェア構成を変更するまで、node4はリブート時に自動的にクォーラムに再参加しない可能性があります。
...
+
詳細については、link:other-references.html["参考資料"] を参照して _Network Management_ または _ONTAP 9 Command reference_ にリンクし、_Configuring failover settings on a LIF_ に進んでください。
