...
元のノードのポートとLIF構成を、新しいノードの構成案と互換性のあるものにする必要があります。これは、新しいノードが起動時に同じ構成を再生するためです。つまり、node3 を起動すると、ONTAP は node1 で使用されていたのと同じポートでLIFをホストしようとします。

したがって、node1の物理ポートがnode3の物理ポートに直接マッピングされていない場合、ブート後にクラスタ、管理、およびネットワーク接続をリストアするには、ソフトウェア構成の変更が必要になります。さらに、node1のクラスタポートがnode3のクラスタポートに直接マッピングされていない場合、正しい物理ポートでクラスタLIFをホストするようにソフトウェア構成を変更するまで、node3は再起動時に自動的にクォーラムに再参加しない可能性があります。
...
|===
+
この情報を入手するための手順については、link:record_node1_information.html["ノード 1 の情報を記録します"]を参照してください。
...
----
+
link:other-references.html["参考資料"]に移動して、詳細については『ネットワーク管理』または『ONTAP 9 コマンド リファレンス』にリンクしてください。
