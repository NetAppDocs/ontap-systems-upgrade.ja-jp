...
.作業を開始する前に
FC スイッチ ポートの使用方法は、RCF を使用できるようにするために、_Fabric-attached MetroCluster Installation and Configuration_ コンテンツに記載されている使用方法と一致している必要があります。_Fabric-attached MetroCluster Installation and Configuration_ コンテンツへのリンクは、link:other-references.html["参考資料"] を参照してください。
