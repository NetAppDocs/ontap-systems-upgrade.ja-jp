...
. [[auto_install4_step27]]NetApp Storage Encryption (NSE) ドライブがインストールされている場合は、次の手順を実行します。
+
...
+
FCまたはUTA / UTA2の設定がない場合は、ノード4がノード2のディスクを認識できるようにを実行しますlink:reassign-node2-disks-to-node4.html#reassign-node2-node4-app-step1["ノード2のディスクをノード4の手順1に再割り当てします。"]。
