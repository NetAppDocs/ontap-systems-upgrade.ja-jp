...
node2 の物理ポートが node4 の物理ポートに直接マッピングされない場合は、次のセクション<<restore_network_configuration_on_node4,ノード 4 のネットワーク設定をリストアします>>を使用してネットワーク接続を修復する必要があります。
...
node4がクォーラムに参加していることを確認してください。次に、コントローラ交換処理を再開します
...
+
到達可能性修復が実行されると、ONTAPはポートを正しいブロードキャストドメインに配置しようと試みます。ただし、ONTAPがポートの到達可能性を判断できず、既存のブロードキャストドメインのいずれにも属していない場合、ONTAPはこれらのポート用に新しいブロードキャストドメインを作成します。
...
.次の手順
node4のインストールを確認した後、link:restore_key-manager_config_node4.html["node4 上のキーマネージャ構成を復元する"]。
