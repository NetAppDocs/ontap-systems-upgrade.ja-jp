...
summary: 'ONTAP 9.8以降を実行しているコントローラをアップグレードするときに、障害が発生したアグリゲートまたは拒否されたアグリゲートをノード2に手動で再配置します。 `system controller replace`コマンド。' 
---
...
.次の手順
失敗した、または拒否された集合体を移動した後、link:retire_node1.html["node1を撤去"]。
