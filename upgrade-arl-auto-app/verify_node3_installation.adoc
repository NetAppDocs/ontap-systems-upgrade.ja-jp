...
node1 の物理ポートが node3 の物理ポートに直接マッピングされない場合は、後続のセクション<<restore_network_configuration_on_node3,ノード 3 でネットワーク設定をリストア>>を使用してネットワーク接続を修復する必要があります。
...
.次の手順
node3のインストールを確認した後、link:restore_key-manager_configuration_node3.html["node3 上のキーマネージャ構成を復元する"]。
