...
summary: 'ONTAP 9.8以降を実行しているコントローラをアップグレードするときに、障害が発生したアグリゲートまたは拒否されたアグリゲートを手動でノード3に再配置します。 `system controller replace`コマンド。' 
---
...
.次の手順
失敗した、または拒否された集合体を移動した後、link:retire_node2.html["node2 を廃止する"]。
