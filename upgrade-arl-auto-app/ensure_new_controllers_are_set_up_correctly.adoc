...
. 構成に自己暗号化ドライブが使用されている場合は、を設定しておきます `kmip.init.maxwait` 変数をに設定します `off` （例：で） link:install_boot_node4.html#step27["Node4 をインストールしてブートします（手順 27 ）"]）を使用している場合は、次のように変数を設定解除
+
