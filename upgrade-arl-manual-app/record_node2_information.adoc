...
NOTE: システムにクラスタ間LIFがない場合があります。クラスタ管理LIFは、ノードペアの一方のノードにのみ存在します。クラスタ管理LIFは、_node1のポート情報を記録_のlink:record_node1_information.html#step1["手順 1."]の出力例に表示されます。
...
+
ブロードキャストドメインは、後の手順で新しいコントローラのポートにマッピングする必要があります。
...
.次の手順
node2情報を記録した後、link:retire_node2.html["node2 を廃止する"]。
