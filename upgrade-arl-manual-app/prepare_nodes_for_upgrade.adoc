...
+
link:other-references.html["参考資料"]を参照して「HAペア管理」にリンクし、「HAペアのベストプラクティス」セクションに従ってください。元のノードはどちらも使用率が50%を超えて稼働してはなりません。ノードの使用率が50%未満であれば、コントローラのアップグレード中に両方のノードの負荷を処理できます。
...
ユーザーアカウントのロック解除については、link:other-references.html["参考資料"]を参照して『システム管理リファレンス』にアクセスしてください。

====
.. link:other-references.html["参考資料"]を参照して、NetApp Support Siteにアクセスし、パフォーマンスおよび統計コレクター（Perfstat Converged）をダウンロードしてください。
+
...
. link:other-references.html["参考資料"]を参照して、NetApp Support Siteにリンクし、NetApp Support Siteでサポートケースを開いてください。
+
アップグレード中に発生する可能性がある問題をケースで報告できます。

. node3 と node4 の NVMEM または NVRAM バッテリが充電されていることを確認し、充電されていない場合は充電します。
+
node3とnode4を物理的にチェックして、NVMEMまたはNVRAMバッテリーが充電済みであることを確認する必要があります。node3およびnode4のモデルのLEDに関する情報については、link:other-references.html["参考資料"]を参照して _Hardware Universe_ にアクセスしてください。
...
+
新しいノードには、元のノードにインストールされているものと同じバージョンの ONTAP 9.x がインストールされている必要があります。新しいノードに異なるバージョンの ONTAP がインストールされている場合は、インストール後に新しいコントローラをネットブートする必要があります。ONTAP のアップグレード方法については、link:other-references.html["参考資料"] を参照して _Upgrade ONTAP_ にリンクしてください。
...
....
+
どちらかのノードが部分的なギブバック状態にある場合は、 `storage failover giveback`コマンドを使用してギブバックを実行し、次に `storage failover show-giveback`コマンドを使用して、まだギブバックが必要なアグリゲートがないことを確認してください。コマンドの詳細については、link:other-references.html["参考資料"]を参照して _HA ペア管理_ にリンクしてください。
...
+
node1もnode2も、自身が現在の所有者である（ただし、ホーム所有者ではない）アグリゲートを所有していない場合、システムは次の例のようなメッセージを返します：

+
....
cluster::> storage aggregate show -node node2 -is-home false -fields owner-name,home-name,state
...
....
+
ディスクが欠落している場合は、link:other-references.html["参考資料"]を参照して、_CLI を使用したディスクおよびアグリゲートの管理_、_CLI を使用した論理ストレージの管理_、および_HA ペアの管理_にリンクし、HA ペアのストレージを設定してください。
...
+
 `network interface revert`コマンドの詳細については、link:other-references.html["参考資料"]を参照して_ONTAP 9 Command reference_にリンクしてください。
...
+
ディスクに障害が発生した場合は、『CLI を使用したディスクおよびアグリゲートの管理』の手順に従って、該当するディスクを取り外してください。（『CLI を使用したディスクおよびアグリゲートの管理』へのリンクは link:other-references.html["参考資料"] を参照してください。）
...
NOTE: この情報を使用して、ノード 3 またはノード 4 に転送する可能性のある部品またはアクセサリを識別できます。
...
+
VLANは物理ポート上で設定されます。物理ポートが変更された場合は、手順の後半でVLANを再作成する必要があります。
...
+
SVMに対してiSCSIもFCも設定されていない場合、コマンドは次の例のようなメッセージを表示します：
...
+
SVMにiSCSIまたはFCのいずれかが設定されている場合、コマンドは次の例のようなメッセージを表示します：
...
....
+
クラスタ内のノードは、ノードの過半数が正常で相互に通信できる状態にあるときに、クォーラム（定足数）を満たしていると言えます。詳細については、link:other-references.html["参考資料"] を参照して『システム管理リファレンス』へのリンクをご確認ください。
...
. [[man_prepare_nodes_step28]]次のサブステップを完了します。
+
...
+
リモート管理デバイス（この場合は SP）のネットワークパラメータを、元のシステムから新しいノード上の SP に再利用することもできます。SP の詳細については、link:other-references.html["参考資料"]を参照して_システム管理リファレンス_および_ONTAP 9 コマンドリファレンス_にリンクしてください。



. [[man_prepare_nodes_step29]]新しいノードに元のノードと同じライセンス機能を持たせたい場合は、次のコマンドを入力して元のシステムのクラスター ライセンスを確認します。
+
...
....
. 新しいノードの新しいライセンスキーは、_NetApp Support Site_で入手してください。_NetApp Support Site_へのリンクについては、link:other-references.html["参考資料"]を参照してください。
+
...
『システム管理リファレンス』の指示に従って、AutoSupport を有効にしてください。（『システム管理リファレンス』へのリンクについては、link:other-references.html["参考資料"] を参照してください。）
...
+
AutoSupportの詳細については、link:other-references.html["参考資料"]を参照して、_System Administration Reference_および_ONTAP 9 Command reference_にリンクしてください。
...
.次の手順
ノードのアップグレード準備後、link:manage_authentication_okm.html["Onboard Key Managerを使用して認証キーを管理する"]。
