...
.. 到達可能性修復が実行されると、ONTAP はポートを正しいブロードキャストドメインに配置しようと試みます。ただし、ONTAP がポートの到達可能性を判断できず、そのポートが既存のブロードキャストドメインのいずれにも対応していない場合、ONTAP はこれらのポート用に新しいブロードキャストドメインを作成しようとします。必要に応じて、新しく作成されたブロードキャストドメインのすべてのメンバーポートがインターフェースグループのメンバーポートになった場合は、それらのドメインを削除できます。ブロードキャストドメインを削除します（：）
+
...
. [[man_map_1_step5]]ONTAP 9.8以降、ONTAPはネットワークポート到達可能性修復手順中にポートがブロードキャストドメイン間で移動された場合、LIFのホームポートを自動的に変更します。LIFのホームポートが別のノードに移動された場合、または割り当てられていない場合、そのLIFは移動されたLIFとして表示されます。ホームポートが存在しなくなった、または別のノードに移動された、移動されたLIFのホームポートを復元します。
+
...
.次の手順
node1からnode3へポートをマッピングした後、link:move_nas_lifs_node1_from_node2_node3_verify_san_lifs_node3.html["node1が所有するNASデータLIFをnode2からnode3に移動し、node3上のSAN LIFを検証する"]。
