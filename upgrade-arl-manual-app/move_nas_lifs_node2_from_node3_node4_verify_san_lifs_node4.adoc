...
[role="lead"]
ポートを node2 から node4 にマッピングしたあと、 node2 のアグリゲートを node3 から node4 に再配置する前に、 node3 の現在の node2 に所有されている NAS データ LIF を node4 に移動する必要があります。ノード 4 の SAN LIF も確認する必要があります。
