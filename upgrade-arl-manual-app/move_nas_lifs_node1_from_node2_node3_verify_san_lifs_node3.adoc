...
.このタスクについて
リモートLIFは、アップグレード手順中にSAN LUNへのトラフィックを処理します。アップグレード中にSAN LIFを移動することは、クラスタやサービスの健全性を維持するために必要ではありません。SAN LIFは、新しいポートにマッピングする必要がない限り、移動されません。node3をオンラインにした後、LIFが正常に動作し、適切なポートに配置されていることを確認する必要があります。
...
.次の手順
node1が所有するNASデータLIFをnode2からnode3に移動した後、node3でSAN LIFを検証し、。
