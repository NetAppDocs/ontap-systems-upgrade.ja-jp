...
summary: ONTAP 9.8 以降を実行しているコントローラで ARL を手動でアップグレードする場合は、ノード 1 のクラスタネットワーク、管理、 FC ポート、および NVRAM システム ID に関する情報を記録します。 
---
= ノード 1 の情報を記録します
:hardbreaks:
:allow-uri-read: 
...
+
ブロードキャストドメインは、手順の後半で新しいコントローラの新しいポートにマッピングする必要があります。
...
.次の手順
ノード1の情報を記録した後、link:retire_node1.html["node1を撤去"]。
