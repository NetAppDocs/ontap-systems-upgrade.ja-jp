...
NOTE: 続行する前に、アグリゲートをオフラインにしてから、内蔵ディスクドライブ上のアグリゲートを削除する必要があります。アグリゲートの管理に関する情報については、link:other-references.html["参考資料"] を参照して _CLI を使用したディスクとアグリゲートの管理_ コンテンツにアクセスしてください。
...
NOTE: 続行する前に、ボリュームを外部のディスク シェルフに移動し、アグリゲートをオフラインにしてから、内蔵ディスクドライブ上のアグリゲートを削除する必要があります。ボリュームの移動に関する情報については、link:other-references.html["参考資料"] を参照して、_CLI によるディスクおよびアグリゲート管理_のコンテンツにリンクしてください。



| 内部ドライブ上のルートボリュームが含まれます | この手順を続行しないでください。コントローラーのアップグレードについては、link:other-references.html["参考資料"] を参照して NetApp Support Site にリンクし、_ボリュームを移動することによってクラスタ化された Data ONTAP を実行しているノード ペアのコントローラ ハードウェアをアップグレードする_ の手順を使用してください。 


| 内蔵ドライブにルート以外のボリュームを格納し、外付けストレージにボリュームを移動することはできません | この手順を続行しないでください。_ボリュームを移動することによってクラスタ化された Data ONTAP を実行しているノードペアのコントローラハードウェアをアップグレードする_ の手順を使用して、コントローラをアップグレードできます。link:other-references.html["参考資料"] を参照して、この手順にアクセスできる _NetApp Support Site_ へのリンクを確認してください。 
|===


.次の手順
コントローラが内部ディスクドライブにアグリゲートを持っているかどうかを判断した後、link:prepare_nodes_for_upgrade.html["ノードをアップグレード用に準備する"]。
