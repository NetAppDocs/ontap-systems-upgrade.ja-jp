...
ストレージアグリゲートの再配置コマンドの詳細については、link:other_references.html["参考資料"]を参照して、_CLI を使用したディスクおよびアグリゲートの管理_および_ONTAP 9 コマンドリファレンス_にリンクしてください。
...
.次の手順
node2の非ルート集約をnode3からnode4に再配置した後、link:manage_authentication_kmip.html["KMIPサーバを使用して認証を管理する"]。
