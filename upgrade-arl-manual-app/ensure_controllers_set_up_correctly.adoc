...
+
link:other-references.html["参考資料"]に移動して、SPに関する情報については_System Administration Reference_を、 `system service- processor network modify`コマンドの詳細については_ONTAP 9 Command reference_を参照してください。

. 新しいノードでスイッチレスクラスタをセットアップする場合は、link:other-references.html["参考資料"]にアクセスして _Network Support Site_ にリンクし、_Transitioning to a two-node switchless cluster_ の手順に従ってください。
...
.次の手順
新しいコントローラーが正しく設定されていることを確認した後、link:set_up_storage_encryption_new_controller.html["新しいコントローラモジュールで Storage Encryption を設定する"]。
